...
- `src/lib/cart-context.tsx` — client-side shopping bag (persisted to
  `localStorage`)
- `src/app/api/create-order` — prices the bag and opens a Razorpay order
  (prices are always read from the server-side catalog, never trusted from
  the client); `src/app/api/verify-payment` checks the signature Razorpay
  returns before an order counts as paid
...
Checkout is powered by **Razorpay Standard Checkout** — cards, UPI, net
banking and wallets, in a modal served by Razorpay, so card details never
reach this site.

Razorpay's window collects payment and nothing else, so `/checkout` gathers
the delivery address, phone and engraving name first and carries them on the
order's `notes`. There is no database: those notes and the Razorpay dashboard
are the order record. Shipping is complimentary worldwide, so no carrier
integration is involved and orders are despatched by hand from those details.

Set `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `NEXT_PUBLIC_RAZORPAY_KEY_ID`
and `NEXT_PUBLIC_SITE_URL` (copy `.env.example` to `.env.local` for local
work). Until the keys are set, the order API returns 503 and checkout falls
back to the contact/concierge flow.

The catalogue is priced in USD but Razorpay bills a single currency — INR
unless International Payments is activated on the account — so
`RAZORPAY_INR_PER_USD` sets the rate the house sells at.
